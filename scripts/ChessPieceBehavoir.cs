...
public class ChessPieceBehavoir : MonoBehaviour
{
    ...
    public void SetPieceData(ChessBoardBehavoir chessboardBehavoir, int column, int row, PieceColor color, PieceType type) {
        _Column = column;
        _Row = row;
        _Color = color;
        _Type = type;
        boardBehavoir = chessboardBehavoir;
    ...
    public bool checkValidMoveFull(int newColumn, int newRow) {return KingUnderAttack(newColumn, newRow);}

    // method to update the board
    public void movePiece(int newColumn, int newRow) {
        boardBehavoir.updateBoardAtPos(_Column, _Row);

        if(boardBehavoir.pieceAtpos(newColumn, newRow)) {boardBehavoir.deletePieceAtPos(newColumn, newRow);}
        else {boardBehavoir.updateBoardAtPos(newColumn, newRow);}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    bool KingMove(int newColumn, int newRow) {
        if(newColumn >= 0 && newColumn < boardBehavoir.gridSize && newRow >= 0 && newRow < boardBehavoir.gridSize) {

            //check if occupied
            if(boardBehavoir.pieceAtpos(newColumn, newRow)) {
                if(boardBehavoir.getPieceAtPos(newColumn, newRow).Color == _Color) {return false;}
            }            
            ...
            for(int i = 0; i < boardBehavoir.gridSize; i++) {
                for(int j = 0; j < boardBehavoir.gridSize; j++) {
                    if(boardBehavoir.pieceAtpos(i, j)) {
                        ChessPieceBehavoir enemyPiece = boardBehavoir.getPieceAtPos(i,j);
    ...
    bool BishopMove(int newColumn, int newRow) {
        if(newColumn != Column && newRow != _Row) {
            ...
            int maxUpRightColumn = boardBehavoir.gridSize;
            int maxUpRightRow = boardBehavoir.gridSize;

            int maxUpLeftColumn = 0;
            int maxUpLeftRow = boardBehavoir.gridSize;

            int maxDownRightColumn = boardBehavoir.gridSize;
            ...
            int row = _Row;  //row
            while(!boardBehavoir.pieceAtpos(column + 1, row + 1) && column + 1 < boardBehavoir.gridSize && row + 1 < boardBehavoir.gridSize) {
                column++; row++;
            }
            //check if next square column+1 row+1 there is an enemy or outside board
            if(boardBehavoir.pieceAtpos(column + 1, row + 1) && column < boardBehavoir.gridSize && row < boardBehavoir.gridSize) {
                if(boardBehavoir.getPieceAtPos(column + 1, row + 1).Color != _Color) {
                    column++; row++;
                }
            }
            ...
            column = _Column;
            row = _Row;
            while(!boardBehavoir.pieceAtpos(column - 1, row + 1) && column >= 0 && row < boardBehavoir.gridSize) {
                column--; row++;
            }
            if(boardBehavoir.pieceAtpos(column - 1, row + 1) && column > 0 && row < boardBehavoir.gridSize) {
                if(boardBehavoir.getPieceAtPos(column - 1, row + 1).Color != _Color) {
                    column--; row++;
                }
            }
            ...
            column = _Column;
            row = _Row; 
            while(!boardBehavoir.pieceAtpos(column + 1, row - 1) && column < boardBehavoir.gridSize && row >= 0) {
                column++; row--;
            }
            if(boardBehavoir.pieceAtpos(column + 1, row - 1) && column < boardBehavoir.gridSize && row > 0) {
                if(boardBehavoir.getPieceAtPos(column + 1, row - 1).Color != _Color) {
                    column++; row--;
                }
            }
            ...
            column = _Column;
            row = _Row; 
            while(!boardBehavoir.pieceAtpos(column - 1, row - 1) && column >= 0 && row >= 0) {
                column--; row--;
            }
            //check if next square column+1 row-1 there is an enemy or outside board
            if(boardBehavoir.pieceAtpos(column - 1, row - 1) && column > 0 && row > 0) {
                if(boardBehavoir.getPieceAtPos(column - 1, row - 1).Color != _Color) {
                    column--; row--;
                }
            }
    ...
    bool RookMove(int newColumn, int newRow) {
        if(newColumn >= 0 && newColumn < boardBehavoir.gridSize && newRow >= 0 && newRow < boardBehavoir.gridSize) {
            if((newColumn != Column && newRow == _Row) || (newColumn == Column && newRow != _Row)) {
                // default maximum allowed move on each direction
                int maxUp = boardBehavoir.gridSize;
                int maxDown = 0;
                int maxRight = boardBehavoir.gridSize;
                int maxLeft = 0;
                ...
                int column = _Column;
                int row = _Row;
                for(int i = boardBehavoir.gridSize - 1; i > _Row; i--) {
                    if(boardBehavoir.pieceAtpos(_Column, i)) {
                        if(boardBehavoir.getPieceAtPos(_Column, i).Color != _Color) {maxUp = i + 1;}
                        else {maxUp = i;}
                    }
                }
                for(int i = 0; i < _Row; i++) {
                    if(boardBehavoir.pieceAtpos(_Column, i)) {
                        if(boardBehavoir.getPieceAtPos(_Column, i).Color != _Color) {maxDown = i;}
                        else {maxDown = i + 1;}
                    }
                }
                for(int i = boardBehavoir.gridSize - 1; i > _Column; i--) {
                    if(boardBehavoir.pieceAtpos(i, _Row)) {
                        if(boardBehavoir.getPieceAtPos(i, _Row).Color != _Color) {maxRight = i + 1;}
                        else {maxRight = i;}
                    }
                }
                for(int i = 0; i < _Column; i++) {
                    if(boardBehavoir.pieceAtpos(i, _Row)) {
                        if(boardBehavoir.getPieceAtPos(i, _Row).Color != _Color) {maxLeft = i;}
                        else {maxLeft = i + 1;}
                    }
                }
    ...
    bool PawnMove(int newColumn, int newRow) {
        if(newColumn >= 0 && newColumn < boardBehavoir.gridSize && newRow >= 0 && newRow < boardBehavoir.gridSize) {
            // special  first move
            if(!_FirstTimeMoved && _Row + 2 == newRow && _Column == newColumn && !boardBehavoir.pieceAtpos(newColumn, _Row + 1)) {
            ...
            if(_Row + 1  == newRow) {
                ...
                if(_Column == newColumn && !boardBehavoir.pieceAtpos(newColumn, _Row + 1)) {
                    if(!_FirstTimeMoved) {_FirstTimeMoved = true;}
                    return true;
                }

                else if(_Column - 1 == newColumn && boardBehavoir.pieceAtpos(newColumn, _Row + 1)) {
                    if(boardBehavoir.getPieceAtPos(_Column - 1, Row + 1).Color != _Color) {
                        if(!_FirstTimeMoved) {_FirstTimeMoved = true;}
                        return true;
                    }
                }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    bool RookAttack(int newColumn, int newRow) {
        if(newColumn >= 0 && newColumn < boardBehavoir.gridSize && newRow >= 0 && newRow < boardBehavoir.gridSize) {
            if((newColumn != Column && newRow == _Row) || (newColumn == Column && newRow != _Row)) {
                ...
                int row = _Row;
                for(int i = boardBehavoir.gridSize - 1; i > _Row; i--) {
                    if(boardBehavoir.pieceAtpos(_Column, i)) {maxUp = i + 1;}
                }
                for(int i = 0; i < _Row; i++) {
                    if(boardBehavoir.pieceAtpos(_Column, i)) {maxDown = i;}
                }
                for(int i = boardBehavoir.gridSize - 1; i > _Column; i--) {
                    if(boardBehavoir.pieceAtpos(i, _Row)) {maxRight = i + 1;}
                }
                for(int i = 0; i < _Column; i++) {
                    if(boardBehavoir.pieceAtpos(i, _Row)) {maxLeft = i;}
                }
    ...
    // if king is under attack after moving any piece then the move is invalid
    bool KingUnderAttack(int newColumn, int newRow) {
        ...
        if(checkValidMove(newColumn, newRow)) {
            ...
            boardPosition.copyChessBoardBehavoir(boardBehavoir);
            ChessBoardPositionPieceData[] pieces = boardPosition.Board.getPiecesOfColor(_Color);
            ...
            boardPosition.setLastPieceMoved(this);
            ...
            for(int i = 0; i < boardPosition.Board.gridSize; i++) {
                for(int j = 0; j < boardPosition.Board.gridSize; j++) {
                    if(boardPosition.Board.pieceAtpos(i, j)) {
                        ChessBoardPositionPieceData enemyPiece = boardPosition.Board.getPieceAtPos(i,j);
                        if(enemyPiece.Color != _Color) {
